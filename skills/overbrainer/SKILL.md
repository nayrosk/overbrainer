...
- Stages and training can run for hours. Run them in the background, and check progress with `overbrainer runs ls` or by reading the files in `data/`.
...
- `[providers.NAME]`: `protocol = "openai"` or `"anthropic"`. The URL and key come from `OVERBRAINER_PROVIDERS__NAME__BASE_URL` and `OVERBRAINER_PROVIDERS__NAME__API_KEY`.
...
It fails with the key and the reason when something is wrong, and names the variable to set when a URL or key is missing.

Details: [configuration](https://github.com/nayrosk/overbrainer/blob/v0.1.1/docs/configuration.md).
...
Many parents (every model on the `anthropic` protocol, and Claude, Gemini and OpenAI models except `gpt-oss`) never return raw reasoning. With `reasoning = true` all their answers are excluded as `no_raw_reasoning`, and overbrainer warns at startup. If that warning appears, tell the user before running `answers`.

Details: [the dataset pipeline](https://github.com/nayrosk/overbrainer/blob/v0.1.1/docs/pipeline.md).
...
The job runs detached. Ctrl-C, a closed terminal or a lost connection only stop following it; the training goes on, and overbrainer prints the `overbrainer train attach` command to use. A run ends as `succeeded`, `failed` or `cancelled`. Its files are in `runs/RUN_ID/`: `job.log` (the job's output, read it when a run fails), `metrics.jsonl`, and `output/` with the LoRA adapter (or the full model, and `output/merged/` with `merge = true`).

Details: [training](https://github.com/nayrosk/overbrainer/blob/v0.1.1/docs/training.md) and [Runpod](https://github.com/nayrosk/overbrainer/blob/v0.1.1/docs/runpod.md).

## Troubleshooting

- `config check` fails: fix the key it names in `overbrainer.toml`, or ask the user to set the variable it names. Never read `.env` to find out why.
...
- A run is `failed`: read `runs/RUN_ID/job.log` and the message in `overbrainer runs ls`.
